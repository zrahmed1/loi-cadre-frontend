...
getEtablissementNomByMouvement(_t97: Mouvement) {
    return this.etablissements.find(e => e.id === _t97.posteConcerne?.etablissement?.id)?.nom || 'N/A';
}
  ...
  ngOnInit() {
    ...
    this.utilisateurService.getAll().subscribe(users => this.stats.utilisateurs = users.length);
    this.etablissementService.getAll().subscribe(etabs => this.stats.etablissements = etabs.length);
  ...
  loadFilteredData() {
    ...
    this.mouvementService.getAll().subscribe(mouvs => {
      this.mouvements = mouvs.filter(m => 
        (!etablissementId || m.posteConcerne?.etablissement?.id === etablissementId) &&
        (!departementId || m.posteConcerne?.etablissement?.departement?.id === departementId)
      );
    });
  }
}