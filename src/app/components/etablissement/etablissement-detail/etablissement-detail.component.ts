...
export class EtablissementDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.etablissementService.getById(id).subscribe(etab => {
      ...
      this.posteBudgetaireService.getByEtablissement(id).subscribe(postes => this.postes = postes);
      this.mouvementService.getAll().subscribe(mouvs => {
        this.mouvements = mouvs.filter(m => m.posteConcerne?.etablissementId === id);
      });
    });
  }

  addMouvement() {
    this.router.navigate(['/mouvement/create'], { queryParams: { etablissementId: this.etablissement?.id } });
  }
}